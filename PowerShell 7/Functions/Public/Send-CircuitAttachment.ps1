...
Param (
        ...
        $body.Add("content", "$Message")
                ...
                $Upload = Invoke-RestMethod $uriupload -Method 'POST' -Headers $headers -InFile $FilePath
                ...
                $GetProxy = powershell.exe -command "& {[System.Net.WebRequest]::GetSystemWebproxy().GetProxy('$uriupload').OriginalString}"
                $Upload = Invoke-RestMethod $uriupload -Method 'POST' -Headers $headers -InFile $FilePath -Proxy $GetProxy -ProxyUseDefaultCredentials
                $AttachmentID = $Upload.attachmentId
                $body.Add("attachments",$AttachmentID)
