...
Param (
        ...
        $Commands = Get-ChildItem "$Dir\PowerShell 7\!Commands\*.ps1"
...
#Proxy True starts here:
            $script:GetProxy = powershell.exe -command "& {[System.Net.WebRequest]::GetSystemWebproxy().GetProxy('$uri').OriginalString}"
            while($true) {
            $TimeNow = Get-Date
            Write-Host "ProdBot is listening, it's only $TimeNow"
